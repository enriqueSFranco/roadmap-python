...
class Parent:
    hair_color = "black"
    speaks = ["English", "Spanish", "Japan", "Chine"]


# hereda los métodos y atributos de clase e instancia
class Child(Parent):
    def __init__(self):
        super().__init__()
        self.speaks.append("German", "French", "Italian")

    pass


# Anular los atributos
...
class Student(Person):
    ...
    def __str__(self) -> str:
        return "Name:{}\nLast Name:{}\nAge: {}\nSubjects: {}".format(
            self.name, self.last_name, self.age, self.courses, self.subjects
        )
...
class Vehicle:
    ...
    def drive(self, distance: int):
        if self.is_running:
            ...
            print(
                f"The {self.year} {self.brand} {self.model} has been driven {distance} miles."
            )
        else:
    ...
    def vehicle_info(self) -> str:
        return "Brand: {}\nModel: {}\nYear: {}\nMileage: {}km\nRunning: {}".format(
            self.brand, self.model, self.year, self.mileage, self.is_running
        )
...
herencia = issubclass(
    Car, Vehicle
)  # devolvera true si hereda de la clase Vehicle de lo contrario devolvera false
